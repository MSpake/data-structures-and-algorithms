...
const forEachTwoToThe = (arr) => {
    const newArray = [];
    arr.forEach(element => newArray.push(2 ** element));
    return newArray;
};
...
const extractAbilities = (arr) => {
    return arr.map(abilityObject => abilityObject.ability.name);
};
...
const extractStats = (arr) => {
    return arr.map(statObject => {
        var total = statObject.effort + statObject.baseStat;
        return { name: statObject.stat.name, total: total };
    })
};
