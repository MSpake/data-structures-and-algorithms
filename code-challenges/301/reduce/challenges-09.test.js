...
const returnNames = (arr) => {
  let characters = arr.reduce((accumulator, characterObj) => {
    accumulator.push(characterObj.name);
    return accumulator;
  }, []);
  return characters;
};
...
const reversedString = (arr) => {
  let reversed = Array.from(arr).reduce((accumulator, letter) => {
...
const countNumberOfChildren = (arr) => {
  let children = arr.reduce((accumulator, characterObj) => {
    if (characterObj.children) accumulator += characterObj.children.length;
    return accumulator;
  }, 0);
  return children;
};
...
const calculateAverage = (arr) => {
  let totals = arr.reduce((accumulator, val) => {
    accumulator.count++;
    accumulator.sum += val;
    return accumulator;
  }, { count: 0, sum: 0 });
...
const extractStat = (statName, arr) => {
  let statMatch = arr.reduce((accumulator, val) => {
    if (val.stat.name === statName) accumulator = val;
    return accumulator;
  }, {});
  return statMatch;
};
...
const extractChildren = (arr) => {
  let filteredArr = arr.filter(character => character.name.match(/a/g));
  let allChildren = filteredArr.reduce((accumulator, character) => {
    if (character.children) character.children.forEach(name => accumulator.push(name));
    return accumulator;
  }, []);
  return allChildren;
};
