...
const citiesAtoJ = (arr) => {
  const matches = [];
  arr.forEach(city => { if (city.match(/^([A-J])\w*/)) matches.push(city); });
  return matches;
};
...
const matchMonth = (input) => {
  ...
  const new_str = input.replace(/O/, 'o');
  if (/^october$/.test(new_str)) return true;
  if (/^oct$/.test(new_str)) return true;
};
