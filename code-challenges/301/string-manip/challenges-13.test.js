...
const standardizePhoneNumbers = (arr) => {
  return arr.map(number => {
    return `${number.substring(1, 4)}${number.substring(6, 9)}${number.substring(10)}`;
  });
};
...
const allHappy = (arr) => {
  const happy = arr.map(string => string.includes(':)'));
  if (happy.includes(false)) return false;
  return true;
};
...
const findEvery = (arr, target) => {
  const check = arr.map(string => string.includes(target));
...
const unenrollBrook = (arr) => {
  return arr.map(roster => roster.filter(student => {
    if (!student.includes('Brook')) return student;
  }));
};
...
const sortByDay = (arr) => {
  return daysOfWeek.map(day => arr.filter(activity => activity.includes(day)));
};
