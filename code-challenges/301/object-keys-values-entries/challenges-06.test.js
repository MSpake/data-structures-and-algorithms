...
const houseSize = (arr) => {
  ...
  arr.forEach(entry => {
    ...
    for (let i in values) {
      if (values[i]) {
        if ((typeof values[i]) !== 'string') people += values[i].length;
        else people += 1;
      }
    }
    sizes.push({ house: values[3], members: people });
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(entry => {
    ...
    let people = -1;
    for (let i in values) {
      if (values[i] && !(deceasedSpouses.includes(values[i]))) {
        if ((typeof values[i]) !== 'string') people += values[i].length;
        else people += 1;
      }
    }
    survivors.push({ house: values[3], members: people });
  });
  return survivors;
};
