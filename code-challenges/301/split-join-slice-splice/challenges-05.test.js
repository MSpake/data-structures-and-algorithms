...
const listFoods = (recipe) => {
  ...
  for (let i in recipe.ingredients) {
    const reg = recipe.ingredients[i].match(/\d+\s(\S)*\s/);
    const start = reg[0].length;
...
const splitFoods = (recipe) => {
  let result = [];
  for (let i in recipe.ingredients) {
    result.push(recipe.ingredients[i].split(/\d+\s(\S)*\s/)[2]);
  }
  return result;
};
...
const stepActions = (recipe) => {
  ...
  for (let i in recipe.steps) {
    const reg = recipe.steps[i].match(/(\S)*/);
    const end = reg[0].length;
...
const totalSumCSV = (str) => {
  let total = 0;
  const arr = str.split(',');
  for (let i in arr) total += parseInt(arr[i]);
  return total;
};
