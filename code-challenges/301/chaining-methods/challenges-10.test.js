...
const count = (target, input) => {
  return input.reduce((accumulator, arr) => {
    arr.forEach(val => {
      if (val === target) accumulator.push(val);
    });
    return accumulator;
  }, []).length;
};
...
const totalSum = (input) => {
  return input.reduce((accumulator, arr) => {
    accumulator += arr.reduce((sum, val) => {
      sum += val;
      return sum;
    });
    return accumulator;
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => {
    return arr.filter(val => (typeof val) === 'number').filter(byFive => !(byFive % 5)).map(num => (2 ** num));
  });
};
...
let findMaleAndFemale = (data) => {
  return data.filter(obj => { if (obj.gender === 'female' || obj.gender === 'male') return obj.name; }).map(character => character.name).join(' and ');
};
...
let findShortest = (data) => {
  return data.reduce((accumulator, character) => {
    if (character.height > accumulator.height) accumulator = character;
    return accumulator;
  }, { height: 0 }).name;
};
